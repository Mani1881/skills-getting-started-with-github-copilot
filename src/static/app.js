      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length) {
          participantsHtml = `<ul class="participants-list">${details.participants
            .map((p) =>
              `<li class="participant-item"><span class="participant-email">${p}</span><button class="delete-participant" title="Remove participant" data-activity="${encodeURIComponent(
                name
              )}" data-email="${encodeURIComponent(
                p
              )}"><span aria-hidden="true">🗑️</span></button></li>`
            )
            .join("")}</ul>`;
        } else {
        ...
        btn.addEventListener("click", async (e) => {
          ...
          try {
            const response = await fetch(
              `/activities/${activity}/participant/${email}`,
              { method: "DELETE" }
            );
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Show a specific message for duplicate registration
